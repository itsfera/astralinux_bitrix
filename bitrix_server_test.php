<?php
#####################################
define('LAST_MODIFIED','03.02.2012');
#####################################
...
if ($lang=='en')
{
	...
	$M['PHP_VER_DESC']='Required version: 5.0 and higher, 5.2 is recommended'; 
	...
	$M['PHP_ACC_DESC']='PHP Accelerator is recommended (APC, eAccelerator or any other), it allows to greatly reduce the CPU load and PHP scripts execution time. It\'s desirable that the accelerator memory should be sufficient for commonly-used PHP pages. Using of filters is recommended: 
	eaccelerator.filter  !*/help/* !*/admin/* !*/bitrix/*cache/* */bitrix/* 
	*/.*.php<br>If there is no PHP accelerator, analysis of <a 
	href="?phpinfo=Y">phpinfo()</a> is required';
	$M['CONFLICT']='conflict';
	$M['NOT_FOUND']='not found';
	$M['CBP_VALUE']='allow_call_time_pass_reference value';
	...
	$M['MYSQL_REQ']='MySQL 4.1.11 or
	5.0 and higher is highly recommended. (No alpha or beta releases are allowed).'; 
	$M['MYSQL_REQ_D']='MySQL 4.1.11 is highly recommended';
...
else
{
	...
	$M['PHP_VER_DESC']='Требуется 5.0 и выше, рекомендуется 5.2';
	...
	$M['MYSQL_REQ']='Минимальные требования: 4.1.11, рекомендуется 5.0 и выше. Альфа и бета версии не допускаются.';
	$M['MYSQL_REQ_D']='';
...
{
	$r=eaccelerator_info();
	$res = "EA (ver. ".$r['version'].", mem. ".$r['memorySize'].")";
}
...
debug(__LINE__);
$val=intval(ini_get('allow_call_time_pass_reference'));
show(array(GM('CBP_VALUE'),'',1),$val,!$val);
?>
...
// Regex functions
debug(__LINE__);
$val=intval(function_exists("eregi"));
$pr=array(GM('EREGS'),'',1);
show($pr,$val,!$val);
